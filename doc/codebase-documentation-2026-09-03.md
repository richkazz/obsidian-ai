...
## Overview
Obsidian AI is an open-source, full-stack AI agent management, orchestration, and automation platform. It features a FastAPI backend (Python 3.12) supporting dual database persistence (SQLite and MongoDB), a Next.js 16 frontend (React 19, Tailwind CSS 4, Zustand) with NextAuth v5, and a Node.js WhatsApp Web sidecar (`wa-bridge`). The architecture follows a client-server model with real-time Server-Sent Events (SSE) streaming for chat execution, background worker task scheduling via APScheduler, end-to-end AES payload encryption for auth credentials, and containerized Docker sandboxing for isolated code execution. Entry points for execution are `backend/main.py` for API services and background jobs, `frontend/app/layout.tsx` / `frontend/proxy.ts` for web application rendering and API proxying, and `wa-bridge/index.js` for external WhatsApp integration.
...
- **What it does:**
  ...
  2. Server receives `EncryptedPayload`, decrypts using `crypto_utils.py` → `decrypt_payload()` with `ENCRYPTION_KEY`.
  3. Validates unique username/email against database (`User` table / `users` Mongo collection).
  4. Hashes password using `bcrypt` (`auth.py` → `get_password_hash()`).
  5. Creates user record with default role `guest` and permissions.
- **Touches:** `crypto_utils.py`, `auth.py`, `models.py` / `models_mongo.py`.
- **Inputs/Outputs:**
...
### 2. User Authentication & JWT Generation
...
- **Touches:** `auth.py`, `crypto_utils.py`, `pyotp`, `python-jose`.
- **Inputs/Outputs:**
...
### 3. Real-Time Streaming Chat Execution (Agent/Team turn)
...
- **What it does:**
  ...
  6. Constructs LLM provider instance via `provider_factory.py` -> `create_provider_from_config()`.
...
- **Touches:** `llm/provider_factory.py`, `mcp_client.py`, `sandbox_tools.py`, `builtin_tools.py`, `rag_service.py`, `memory_router.py`, `traces_router.py`.
- **Inputs/Outputs:**
...
### 4. Human-In-The-Loop (HITL) Approval Resolution
- **Trigger:** User clicks "Approve" or "Deny" on inline approval card in chat UI or top bar notification badge.
- **Entry point:** `frontend/components/ai-elements/hitl-approval.tsx` calls `POST /chat/hitl/{approval_id}/respond` (`backend/routers/chat_router.py` → `respond_hitl_approval()`).
- **What it does:**
...
- **Touches:** `chat_router.py`, `models.py` / `models_mongo.py`.
- **Inputs/Outputs:**
...
### 5. Dynamic Tool Creation & Proposal Review
- **Trigger:** Agent with `allow_tool_creation = True` calls `create_tool` virtual function during chat.
- **Entry point:** `backend/routers/chat_router.py` → handles `create_tool` execution inside `chat()` loop.
- **What it does:**
  ...
  5. User reviews tool proposal card in chat and clicks "Approve" or "Reject" (`POST /chat/tool-proposals/{proposal_id}/respond`).
...
- **Touches:** `chat_router.py`, `tools_router.py`, `models.py` / `models_mongo.py`.
- **Inputs/Outputs:**
  - *Input:* `ToolProposalResponse` (`status: approved | rejected`).
...
### 6. Visual Workflow & Topological DAG Execution Engine
- **Trigger:** User executes workflow manually from `/workflows` page or APScheduler triggers a scheduled workflow cron job.
- **Entry point:** Manual run: `POST /workflows/{id}/run` (`backend/routers/workflow_runs_router.py` → `run_workflow()`). Scheduled run: `scheduler_executor.py` → `run_scheduled_workflow_sqlite()`. Core execution: `dag_executor.py` → `execute_dag()`.
- **What it does:**
...
- **Touches:** `dag_executor.py`, `workflow_runs_router.py`, `scheduler_executor.py`, `models.py` / `models_mongo.py`.
- **Inputs/Outputs:**
...
- **Touches:** `agents_router.py`, `versions_router.py`, `models.py` / `models_mongo.py`.
- **Inputs/Outputs:**
...
### 8. Long-Term Agent Memory Reflection
- **Trigger:** First message in a new agent session when `memory_enabled = True`.
- **Entry point:** `backend/routers/chat_router.py` → triggers background memory extraction inside `chat()`.
- **What it does:**
...
- **Touches:** `memory_router.py`, `chat_router.py`, `models.py` / `models_mongo.py`.
- **Inputs/Outputs:**
...
### 9. WhatsApp Channel Integration & Voice Reply Pipeline
...
- **What it does:**
  ...
  4. If incoming message is a voice note, calls `stt_service.py` → `transcribe_audio()` using Groq Whisper API (`whisper-large-v3-turbo`) or local `faster-whisper` model.
  ...
  6. Executes agent turn headlessly via `services/agent_runner.py` → `run_agent_headless()`.
  7. Strips XML `<artifact>` tags from text reply.
  8. If voice replies are enabled on channel (`voice_reply_enabled = True`):
     - Calls `services/tts_service.py` -> `synthesize_speech()` pipeline (`Qwen3-TTS` on GPU with optional voice cloning sample -> `Pocket TTS` -> `Kokoro`).
...
- **Touches:** `whatsapp_router.py`, `whatsapp_service.py`, `agent_runner.py`, `tts_service.py`, `stt_service.py`, `wa-bridge/index.js`.
- **Inputs/Outputs:**
...
### 10. Docker Sandbox Container Management & Execution
...
- **Touches:** `sandbox_router.py`, `sandbox_tools.py`, Docker engine API.
- **Inputs/Outputs:**
...
### 11. Eval Harness & Test Suite Execution
- **Trigger:** User clicks "Run Eval Suite" on `/evals` page or Prompt Auto-Optimizer triggers evaluation run.
- **Entry point:** `POST /evals/suites/{id}/run` (`backend/routers/eval_router.py` → `run_eval_suite_endpoint()`). Core logic: `eval_engine.py` → `run_eval_suite_background()`.
- **What it does:**
...
- **Touches:** `eval_router.py`, `eval_engine.py`, `agent_runner.py`, `models.py` / `models_mongo.py`.
- **Inputs/Outputs:**
...
### 12. Prompt Auto-Optimizer Pipeline
- **Trigger:** User clicks "Trigger Optimization" on prompt optimizer UI or APScheduler triggers weekly Monday 02:00 sweep.
- **Entry point:** `POST /optimizer/trigger` (`backend/routers/optimizer_router.py` → `trigger_optimization()`). Core pipeline: `optimizer.py` → `start_optimization_sqlite()` / `start_optimization_mongo()`.
- **What it does:**
...
- **Touches:** `optimizer_router.py`, `optimizer.py`, `eval_engine.py`, `agents_router.py`, `prompt_vault_router.py`.
- **Inputs/Outputs:**
...
### 13. Knowledge Base Creation & Document Vector Indexing
- **Trigger:** User uploads file or pastes text document into a Knowledge Base on `/knowledge/[id]`.
- **Entry point:** `POST /knowledge/{id}/documents` (`backend/routers/knowledge_router.py` → `add_kb_document()`). Vector pipeline: `rag_service.py` → `index_kb_document()`.
- **What it does:**
...
- **Touches:** `knowledge_router.py`, `rag_service.py`, `file_storage.py`, vector store engine.
- **Inputs/Outputs:**
...
### 14. MCP Server Protocol Binding & Tool Discovery
- **Trigger:** User adds or tests an MCP server configuration on `/settings` or sidebar.
- **Entry point:** `POST /mcp-servers/{id}/test` (`backend/routers/mcp_servers_router.py` → `test_mcp_server()`). Core transport: `mcp_client.py` → `connect_mcp_server()`.
- **What it does:**
...
- **Touches:** `mcp_servers_router.py`, `mcp_client.py`, MCP Python SDK.
- **Inputs/Outputs:**
...
- **Touches:** `skills_router.py`, `agents_router.py`, `chat_router.py`.
- **Inputs/Outputs:**
